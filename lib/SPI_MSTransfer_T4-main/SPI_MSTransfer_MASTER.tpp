...
//   return buf[0];
// }
//This has been editted by David
SPI_MSTransfer_MASTER_FUNC uint16_t SPI_MSTransfer_MASTER_OPT::transfer16(uint16_t *buffer, uint16_t length, uint16_t packetID) {
  uint16_t buf[length] = {};
  uint16_t RecData = 0;
  ...
  //Serial.println(*buf);
  RecData = process_data(buf, (sizeof(buf)), buf[3]);
  return RecData;
}
...
//This has been changed by David
SPI_MSTransfer_MASTER_FUNC uint16_t SPI_MSTransfer_MASTER_OPT::process_data(uint16_t *buffer, uint16_t length, uint16_t command) {
  //uint16_t checksum = 0, csum_passed = 0;
  uint16_t test = 0;
  length = 1;
  spi_assert();
