...
#define SPI_MSTransfer_MASTER_CLASS template<SPIClass* port, uint8_t cs_pin, uint32_t slave_ID, uint32_t spi_speed = 1000000>
...
SPI_MSTransfer_MASTER_CLASS class SPI_MSTransfer_MASTER : public SPI_MSTransfer_MASTER_Base {
  ...
  private:
    uint16_t process_data(uint16_t *buffer, uint16_t length, uint16_t command);
