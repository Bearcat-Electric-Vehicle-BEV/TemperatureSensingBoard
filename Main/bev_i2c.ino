...
void displayRequestEvent()
{
    ...
    count += 1;
    Wire.write(Speed + int(random(0, 10)));  //speed (in kph)
    Wire.write(Battery_Temp  + int(random(0, 5)));  //battery temp (in degree C)
    Wire.write(Battery_Life  + int(random(0, 5)));  //range left in distance (in km)
    Wire.write(Range_KM  + int(random(0, 5)));  //range left in terms of time (mins)
    Wire.write(Range_Mins  - int(random(0, 5)));  //battery life (percentage)
